...
**最新版本：v1.10.8**（2026-08-29） · [更新说明](RELEASE_NOTES.md) · [开发路线图](docs/dev-roadmap.md)
...
| 测试 | Vitest（235 项单元/集成测试）+ 端到端验证脚本 |
...
```
src/
├── main/          # Electron 主进程：数据库(迁移 v1~v22 + 服务层)、IPC(189 频道 + 校验 + 门禁)、
...
| 版本 | 日期 | 重点 |
|------|------|------|
| v1.10.8 | 2026-08-29 | 成本价维护重放校准（修复删除/编辑交易后均价漂移） |
