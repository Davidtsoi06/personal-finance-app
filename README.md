...
**最新版本：v1.10.1**（2026-08-16） · [更新说明](RELEASE_NOTES.md) · [开发路线图](docs/dev-roadmap.md)
...
**v1.10.1（2026-08-16）· 港银日结单 + 净值随价更新 + 股票查询**：支持支出/收入分列 + 结余、无方向列的银行日结单（自定义格式修复 + 日期解析增强 + 定期英文关键词）；净值走势随股价/汇率更新即时同步；总览面板改名「股票查询与分析」（只显示现有持仓）；AI 生成模板可直接上传 Excel/CSV；导入时确认目标银行账户/卡号。详见 [RELEASE_NOTES.md](RELEASE_NOTES.md)。
...
| 测试 | Vitest（197 项单元/集成测试）+ 端到端验证脚本 |
