...
**最新版本：v1.10.6**（2026-08-16） · [更新说明](RELEASE_NOTES.md) · [开发路线图](docs/dev-roadmap.md)
...
| 测试 | Vitest（223 项单元/集成测试）+ 端到端验证脚本 |
...
```
src/
├── main/          # Electron 主进程：数据库(迁移 v1~v22 + 服务层)、IPC(188 频道 + 校验 + 门禁)、
...
| 版本 | 日期 | 重点 |
|------|------|------|
| v1.10.6 | 2026-08-16 | 支付宝多区域 + 微信/支付宝账单导入 + 归档资产快照 + AI 会话/报告归档 |
