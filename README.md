...
**最新版本：v1.10.4**（2026-08-16） · [更新说明](RELEASE_NOTES.md) · [开发路线图](docs/dev-roadmap.md)
...
**v1.10.4（2026-08-16）· AI 样例货币金额误转日期修复**：修复 v1.10.3 引入的回归——货币金额（如 30,000）曾被误判成日期序列号；现按单元格真实格式识别（日期→17/8/2026，金额→数字/HK$25,000.00），金额绝不变日期。详见 [RELEASE_NOTES.md](RELEASE_NOTES.md)。
...
| 测试 | Vitest（204 项单元/集成测试）+ 端到端验证脚本 |
