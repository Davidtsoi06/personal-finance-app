...
**最新版本：v1.9.1**（2026-08-16） · [更新说明](RELEASE_NOTES.md) · [开发路线图](docs/dev-roadmap.md)
...
- 投资收益明细：成本价 / 交易价 / 成交数量 / 市值 / 盈亏明细
- 每日交易报表（含成本价列）一键导出 Excel；完整资产汇总导出（多 sheet、CNY 等值）
...
**v1.9.1（2026-08-16）· 成本与迁移修复版**：按历史交易重放一次性重算全部持仓成本价（修正历次加权计算漂移）；投资收益明细新增成本价/交易价/成交数量；数据包导出/导入修复（自洽快照 + 清理残留 WAL，解决导入后启动报 database disk image is malformed）。详见 [RELEASE_NOTES.md](RELEASE_NOTES.md)。

**v1.9.0（2026-08-16）**：定期存款全自动体系（日结单驱动创建/结清 + 双向配对去重 + 防重复导入）；每日交易报表成本价列与盈亏修正。

**v1.8.3（2026-08-16）**：券商日结单手续费智能推算（无成交金额时按数量×价格推成交额）；导入预览全字段可编辑。
...
| 测试 | Vitest（167 项单元/集成测试）+ 端到端验证脚本 |
