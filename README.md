...
**最新版本：v1.10.2**（2026-08-16） · [更新说明](RELEASE_NOTES.md) · [开发路线图](docs/dev-roadmap.md)
...
- 银行日结单导入（CSV / Excel，支持美式 `月/日/年`、`日/月/年`、中文日期与千分位/括号负数金额；支出/收入分列 + 结余、单列带符号均支持）
...
**v1.10.2（2026-08-16）· 持仓过滤 + 导入预览可编辑**：券商持仓明细只显示现有持仓（数量为 0 不再显示）；银行日结单导入预览全字段可编辑（日期/摘要/方向/金额/币种改完再导入）。详见 [RELEASE_NOTES.md](RELEASE_NOTES.md)。
...
| 测试 | Vitest（200 项单元/集成测试）+ 端到端验证脚本 |
