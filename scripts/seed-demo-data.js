...
console.log('');
console.log('===== 种子汇率下的预期金额（供总资产口径验证） =====');
...
console.log('   应用启动后会自动抓取实时汇率覆盖 currencies 表，外币部分金额会随之变化，');
console.log('   因此不要以固定值 ¥713,350 作为验证锚点。');
console.log('');
console.log('验证方式（一致性核对）：');
console.log('   1) $env:PF_USER_DATA_DIR = \'<演示目录>\'; npm start');
console.log('   2) 另开终端运行：node scripts/inspect-summary.js <演示目录>\finance.db');
console.log('   3) 对照脚本输出的「顶级项直接相加」值：资产总览总资产卡 / 概览总资产行 / 资产管理页总资产 / 总资产走势最新一天，四个位置应全部等于该值且随时一致。');
