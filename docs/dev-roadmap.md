...
- [x] **账户归类**：account:ensureAlipayFamily 幂等升级——现有「支付宝」/「支付宝（国内）」账户改名为「支付宝（国内）」挂到新建「支付宝」父账户（余额/流水保留），自动补建「支付宝（香港）」HKD 子账户；兼容已用 createAlipayFamily 建树的用户（不重复建）；资产管理页打开时自动执行
- [x] **银行式展开分组**：getAllAssetsSummary 中带子账户的 e_wallet 父钱包渲染为可展开分组卡片（▶/▼、N 个子账户、余额=自身+子账户合计，不重复计入总资产）；子账户行点击跳钱包流水页 + ✏️ 编辑
- [x] **分组内新建子账户**：「+ 添加支付宝账户」弹窗（名称自定义/币种/初始余额）→ 创建为父账户子账户
