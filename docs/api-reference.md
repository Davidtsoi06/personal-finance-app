...
- AI 接口地址（`ai.apiUrl`）保存前经 `shared/utils/url-safety.ts` 校验：仅允许**公网 HTTPS**，拒绝 `http`、localhost、127.0.0.1、10.x、192.168.x、172.16-31.x、169.254.x 与 IPv6 链路本地地址（防 SSRF）
- 忘记密码的邮箱验证码：60 秒限发、10 分钟有效、5 次试错作废；请求时无论邮箱是否匹配返回一致（防枚举）

## 降级策略

1. 主源失败 → 自动切换到备用数据源
2. 双源均失败 → 使用最近一次成功缓存的数据
3. 数据超过 24 小时未更新，在 UI 上显示"数据可能已过期"提示
4. ⚠️ 已知问题（v1.7.1 审计）：行情/汇率抓取的 fetch 尚未统一接入超时中止，列入 v1.8.0 工程化批次
